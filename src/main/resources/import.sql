...
-- --- Robin Hobb (Fantasy) ---
INSERT INTO libro (id, titolo, isbn, anno_pubblicazione, trama, copertina_url, autore_id, genere_id) VALUES (nextval('libro_seq'), 'L''Assassino di Re', '9788834728461', 1995, 'FitzChevalier, figlio illegittimo di un principe, viene addestrato come assassino al servizio della corona.', '', (SELECT id FROM autore WHERE cognome = 'Hobb'), (SELECT id FROM genere WHERE nome = 'Fantasy'));
INSERT INTO libro (id, titolo, isbn, anno_pubblicazione, trama, copertina_url, autore_id, genere_id) VALUES (nextval('libro_seq'), 'La Nave Ramingo', '9788834728478', 1998, 'Le vicende della famiglia Vestrit e della loro nave vivente, in un mondo di traffici, pirati e creature magiche.', '', (SELECT id FROM autore WHERE cognome = 'Hobb'), (SELECT id FROM genere WHERE nome = 'Fantasy'));
